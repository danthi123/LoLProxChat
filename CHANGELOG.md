...
### Changed
- **"Voice on camera" now goes both ways, and only works between players who have both turned it on.** As first shipped it was listen-only: panning your camera changed what you heard and never what anyone heard from you. That sounded like the conservative choice and was the opposite — it made listening free. You could put your ear anywhere on the map, at no risk, on someone who had no way to know or to decline. Now the place you listen from is a place you can be heard, at the same volume, so listening in on a fight means the people in it hear you; and your client publishes a camera only while the setting is on, with the server using one for a pair only when both sides have published one. **If you leave the setting off, nobody's camera can reach you** — you are heard only by players actually near you on the map, whatever anyone else chooses. Still off by default. Two side effects worth knowing: volumes between any two players are now always symmetric, and the camera *adds* a listening point rather than moving the one you had, so glancing across the map no longer cuts out whoever you are standing next to.
