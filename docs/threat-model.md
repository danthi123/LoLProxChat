...
### "Voice on camera" widens the stock client's own reach (opt-in, mutual)
...
- It does **not** widen the side channel beyond what a *modified* client could already do. `myPosition` in the `/compute-volumes` request has always been client-asserted and unverifiable (see above), so a modified client could already ask "what would I hear from over there?" about any point on the map. The feature gives that to honest clients too, and changes nothing about the attacker's ceiling.
- **It is mutual, and that is the part with teeth.** As shipped in v0.5.8 it was listen-only: the camera scored only the requester's own response, and peers' distances were measured against the requester's champion. That read as the conservative choice and was the opposite — it made listening free and undeclinable. Since v0.5.9 a published camera is both a point you hear from *and* a point you are audible at, and it is used for a pair only when **both** players have published one. Two consequences worth being precise about:
  - **A player who leaves the setting off cannot be reached by anyone's camera.** Their client publishes none, so the pair falls back to champion-to-champion in both directions. This is the property that lets someone opt out of the fog-of-war widening unilaterally, rather than only opting out of exercising it.
  - **Volumes are symmetric by construction.** `V(A,B) = V(B,A)` for every pair: each side contributes the same point set to the same closest-approach calculation (`closestApproach` in `server/src/volumes.ts`). Asymmetric audibility is now a bug, not a configuration.
- **There is no consent flag on the wire to forge.** Presence of the camera in room state *is* the opt-in, and the server reads the requester's own camera from that same stored copy rather than from the request. So a client cannot listen from a point its peers are not scored against. The pre-v0.5.9 `listenPosition` request field is ignored; `server/tests/volumes.test.ts` and `server/tests/integration.test.ts` both assert that sending it buys nothing, including against a peer who has opted out.
- A modified client can of course still publish a false camera — but it is then audible at that false point, and it still cannot reach a peer who published none. Lying costs the liar the same thing it costs an honest user.
- The team filter and `MAX_HEARING_RANGE` cutoff still apply from every point — the server remains the authority for both.
